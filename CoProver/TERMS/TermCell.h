...
private:
    static TermCell* parse_cons_list(Scanner* in, Sigcell* sig, VarBank* vars);
    TermCell* term_check_consistency_rek(SplayTree<PTreeCell> &branch, DerefType deref);
    void print_cons_list(FILE* out, Sig_p sig, DerefType deref);
public:
    /*---------------------------------------------------------------------*/
...
public:
    ...
    void TermPrint(FILE* out, Sig_p sig, DerefType deref);
    void TermPrintArgList(FILE* out, int arity, Sig_p sig, DerefType deref);
    ...
    static inline TermCell* TermDeref(TermCell* term, DerefType deref) {
        ...
        if (deref == DerefType::DEREF_ALWAYS) {
            while (term->binding) {//debug 20170511 该替换项替换成本身 造成死循环 
                term = term->binding;
            }
        } else {
    ...
    static int TermParseArgList(Scanner* in, TermCell*** arg_anchor, Sigcell* sig, VarBank* vars);
    static TermCell* TermParse(Scanner* in, Sig_p sig, VarBank* vars);
